...
exports.onCreateNode = function ({ actions: gatsbyActions, node, createContentDigest }) {
  ...
  } else if (node.internal.type === `Site` && node.siteMetadata && node.siteMetadata.navigation) {
    const navigation = node.siteMetadata.navigation
    navigation.forEach(navItem => createSiteNavigationNode(node, navItem, actions))
  }
}